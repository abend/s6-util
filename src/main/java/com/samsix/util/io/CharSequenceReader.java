...
/**
 * Similar in concept to a StringReader, useful for reading from StringBuffer/StringBuilder
 * without the memory overhead of creating a String.
 *
 * Behavior is undefined if you modify the sequence. Don't do that.
 */
        ...
        Reader
{
        ...
        throws IOException
    {
        synchronized ( lock )
        {
            ...
            {
                ( (StringBuffer) seq ).getChars( next, next + n, cbuf, off );
            }
            else if( seq instanceof StringBuilder )
            {
                ( (StringBuilder) seq ).getChars( next, next + n, cbuf, off );
            }
            else if( seq instanceof String )
            {
                ( (String) seq ).getChars( next, next + n, cbuf, off );
            }
            else
            {
                seq.subSequence( next, next + n ).toString().getChars( 0, n, cbuf, off );
            }
